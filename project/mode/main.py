    ...
    try:
        ...
        async with aiohttp.ClientSession(trust_env=True) as session:
            response = await session.get(
                f'https://ref.mode.network/pre-check-points/{address}',
                params=params,
                proxy='http://' + proxy if proxy else None)

            response = await response.json()

            await excel_write(address=address, quantity=int(response),
                              excel=excel)
...
def main_mode(wallets: list[str], excel, proxy):
    ...
    excel.workbook.save('result/Mode.xlsx')
