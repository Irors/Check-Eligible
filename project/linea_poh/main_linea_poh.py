    ...
    try:
        async with aiohttp.ClientSession(trust_env=True) as session:
            response = await session.get(
                f'https://linea-xp-poh-api.linea.build/poh/{address.lower()}',
                proxy='http://' + proxy if proxy else None)

            data = await response.content.read()
            parsed = json.loads(data)
            await excel_write(address=address, quantity=parsed['poh'], excel=excel)
...
def main_linea_poh(wallets: list[str], excel, proxy):
    if proxy:
    ...
    excel.workbook.save('result/Linea-POH.xlsx')
